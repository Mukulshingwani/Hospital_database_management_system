...
var Main = (function () {
	...
	var sidebarHandler = function () {
		...
		$(document).on("mousedown touchstart", function (e) {
			if (
				...
				!elem.is(e.target) &&
				!sidebarMobileToggler.is(e.target) &&
				($("#app").hasClass("app-slide-off") ||
					$("#app").hasClass("app-offsidebar-open"))
			) {
				resetSidebar();
	...
	var toggleClassOnElement = function () {
		...
		toggleAttribute.each(function () {
			...
			var toggleOutside = function (e) {
				if (
					outsideElement.has(e.target).length === 0 &&
	...
	var settingsHandler = function () {
		...
		$('input[name="setting-theme"]').change(function () {
			var selectedTheme = "theme-2"
			$("#skin_color").attr(
				"href",
				"assets/css/themes/" + selectedTheme + ".css"
			);
			switchLogo(selectedTheme);
			"theme-2" = selectedTheme;
